...
public class TestSimulatorOps {
	...
	protected void doJacobian(RealMatrix jacobian, Position position) {
		...
		jacobian.setRow(0, new double[] { cPsi * cTheta, -sPsi * cPhi + cPsi * sTheta * sPhi,
				sPsi * sPhi + cPsi * sTheta * cPhi, 0d, 0d, 0d });
		jacobian.setRow(1, new double[] { sPsi * cTheta, cPsi * cPhi + sPsi * sTheta * sPhi,
...
class VelocityImpl implements Velocity {
	...
	public double getSpeed() {
		speed = Math.sqrt(Math.pow(getBodyVelX(), 2) + Math.pow(getBodyVelY(), 2) + Math.pow(getBodyVelZ(), 2));
		return speed;
	}
...
class AccelerationImpl implements Acceleration {
	...
	public double getMagnitude() {
		magnitude = Math
				.sqrt(Math.pow(getBodyAccelX(), 2) + Math.pow(getBodyAccelY(), 2) + Math.pow(getBodyAccelZ(), 2));
		return magnitude;
	}
...
class Route {
	...
	// segments from a road? what is the granularity on distance between each
	// point?
